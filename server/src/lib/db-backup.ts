...
async function writeTarget(target: string, payload: Buffer): Promise<void> {
  ...
  if (!restrictToOwner(target)) console.warn(`[db-backup] could not restrict permissions on ${target} — it may be readable by other local accounts`);
}
...
export async function restoreDbBackupIfNeeded(dbPath = getDefaultDbPath()): Promise<DbBackupResult> {
  ...
  let restored: Buffer;
  try { restored = gunzipSync(decryptBackup(payload)); }
  catch (err) { throw new Error(`could not restore SQLite backup from ${target}: ${err instanceof Error ? err.message : String(err)}`); }
